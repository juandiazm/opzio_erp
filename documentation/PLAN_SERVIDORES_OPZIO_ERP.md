# Plan de acción V2 - Servidores de infraestructura integrada con `opzio_erp`
...
El servidor prepara las **fuentes de servidores** y los permisos necesarios:
...
- ningún endpoint de servidores del agente o del ERP queda expuesto públicamente.
...
Agent
  recolecta + normaliza
        │
        ▼
POST /api/internal/servers/v1/ingest
        │
        ▼
opzio_erp
...
```text
GET /api/internal/servers/v1/config
```

Y reportará salud mediante:

```text
POST /api/internal/servers/v1/heartbeat
```
...
```text
opzio_erp/docs/servers/SERVER_CHANGELOG.md
```
...
```text
docs/servers/server-baseline.md
```
...
```text
/etc/nginx/conf.d/opzio_servers.conf
```

Contenido base:

```nginx
log_format opzio_servers escape=json
    '{'
      '"ts":"$msec",'
      '"host":"$host",'
...
```nginx
access_log /var/log/nginx/opzio/iac_main.access.json opzio_servers;
```

Otro proyecto:

```nginx
access_log /var/log/nginx/opzio/clean_express.access.json opzio_servers;
```
...
sudo nginx -t
sudo systemctl reload nginx
curl -i http://127.0.0.1:9080/api/internal/servers/v1/health
```
...
```ini
[Unit]
Description=OPZIO Servers Agent
...
Solo añadir al repositorio las piezas necesarias para servidores y verificar que el pipeline existente:
...
# 21. ERP-001 - Módulo de servidores
...
servers_hosts
servers_projects
servers_project_domains
servers_host_samples
servers_project_samples
servers_http_buckets
servers_storage_samples
servers_events
servers_agents
servers_rollups_hourly
servers_rollups_daily
```
...
`servers_projects` debe almacenar al menos:
...
```text
BuildServersHourlyRollups
BuildServersDailyRollups
PruneServersRawData
```
...
```bash
php artisan servers:deployment-record \
  --component=opzio_erp \
  --sha="$GITHUB_SHA" \
  --status=completed
```
...
```text
# Servers - ERP Agent Rules

1. Read docs/servers/PLAN.md and PROGRESS.md first.
...
6. Internal servers endpoints must remain loopback-only at infrastructure level.
...
```text
feat/erp-005-servers-ingest
feat/erp-010-servers-dashboard
```
