...
use App\Exportable\observability_projects;
use Carbon\Carbon;
...
class observability_dashboard_controller extends Controller
{
    ...
    public function get_page(Request $request)
    {
        ...
        return response()->json([
            ...
            'filters' => [
                'hosts' => observability_host::where('enabled', true)
                    ->orderBy('name')
                    ->get(['key', 'name'])
                    ->values(),
                'environments' => observability_project::where('enabled', true)
                    ->distinct()
                    ->orderBy('environment')
                    ->pluck('environment')
                    ->values(),
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(
            new observability_projects($rows),
            'observabilidad-' . now()->format('Y-m-d_H-i') . '.xlsx'
        );
    }
    ...
    private function projectSummary($project, $from, int $minutes): array
    {
        ...
        $sample = DB::table('observability_project_samples')
            ...
            ->orderByDesc('sampled_at')
            ->first();
        $firstPeriodSample = DB::table('observability_project_samples')
            ->where('project_id', $project->id)
            ->where('sampled_at', '>=', $from)
            ->orderBy('sampled_at')
            ->first();
        $lastPeriodSample = DB::table('observability_project_samples')
            ->where('project_id', $project->id)
            ->where('sampled_at', '>=', $from)
            ->orderByDesc('sampled_at')
            ->first();
        $sampleStats = DB::table('observability_project_samples')
            ->where('project_id', $project->id)
            ->where('sampled_at', '>=', $from)
        ...
        $latestBucket = DB::table('observability_http_buckets')
            ...
            ->first();
        $peakBucket = DB::table('observability_http_buckets')
            ->where('project_id', $project->id)
            ->where('bucket_start', '>=', $from)
            ->orderByDesc('requests_total')
            ->first();
        $latestStorage = DB::table('observability_storage_samples')
            ->where('project_id', $project->id)
            ->orderByDesc('sampled_at')
            ->first();
        $firstPeriodStorage = DB::table('observability_storage_samples')
            ->where('project_id', $project->id)
            ->where('sampled_at', '>=', $from)
            ->orderBy('sampled_at')
            ->first();
        $lastPeriodStorage = DB::table('observability_storage_samples')
            ->where('project_id', $project->id)
            ->where('sampled_at', '>=', $from)
            ->orderByDesc('sampled_at')
            ->first();
    ...
    {
        $projects = observability_project::with(['host.agents'])
            ->where('enabled', true)
