...
export function addDepartment(onCreated){
    ...
    if(directorId == null || directorId == 0){ $('#add-department-director').addClass('is-invalid'); alertWarning('Debe seleccionar un director'); flag = false; }else $('#add-department-director').removeClass('is-invalid');
    if(flag){
        ...
        PostMethodFunction('/admin/departments/add',{name,budget,director_id:directorId},null,function(response){
            $('#add-department-button').prop('disabled', false); $('#create-department-name').val(''); $('#create-department-budget').val(''); $('#create-department-director').val(0);
