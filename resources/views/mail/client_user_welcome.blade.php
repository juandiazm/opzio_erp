...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .restore-code { font-size: 36px; font-weight: 700; color: #220245; letter-spacing: 4px; font-family: 'Courier New', Courier, monospace; display: block; text-align: center; padding: 8px 0; text-decoration: none; }
...
<div class="email-wrapper">
    <div class="email-container">
        ...
        <div class="email-content">
            <h1 class="email-greeting">¡Bienvenido a Opzio!</h1>
            <p class="email-text">Hola <strong>{{ $Data['name'] }}</strong>, tu cuenta ha sido creada exitosamente. Usa el código temporal a continuación para acceder:</p>
            <div class="info-box primary">
                <p class="info-label">Tu código temporal de acceso</p>
                <a href="{{ env('APP_URL') }}client?restore-email={{ $Data['email'] }}&restore-code={{ $Data['password'] }}" class="restore-code" target="_blank">{{ $Data['password'] }}</a>
            </div>
            <div class="notice-box">
                Debes ingresar este código en la aplicación antes de las <strong>{{ \Carbon\Carbon::parse($Data['reset_password_date'])->format('Y-m-d H:i') }}</strong>. Después de ese tiempo, deberás solicitar un nuevo código.
            </div>
        </div>
