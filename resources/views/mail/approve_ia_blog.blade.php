    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media only screen and (max-width: 600px) {
            ...
            .email-button, .email-button-secondary { display: block; width: 100%; text-align: center; margin: 8px 0; }
...
<div class="email-wrapper">
    <div class="email-container">
        ...
        <div class="email-content">
            <h1 class="email-greeting">Blog pendiente de aprobación</h1>
            <p class="email-text">El sistema ha generado un nuevo artículo de blog con inteligencia artificial. Revisa el contenido y apruébalo si consideras que está correcto.</p>
            <img src="{{ $Data['blog']['image_path'] }}" alt="{{ $Data['blog']['title'] }}" class="post-image">
            <div class="info-box primary">
                <div class="info-box-title">Detalles del artículo</div>
                <div class="info-item">
                    <div class="info-label">Título</div>
                    <div class="info-value">{{ $Data['blog']['title'] }}</div>
                </div>
            </div>
            <p class="email-text">{{ $Data['blog']['brief'] }}</p>
            <div class="divider"></div>
            <div class="email-button-container">
                <a href="{{ config('app.APP_URL').'api/blog/approve/'.$Data['blog']['unique_id'] }}" class="email-button" target="_blank">Aprobar Blog</a>
                <a href="{{ config('app.APP_HOME_PAGE_URL').'blogs/view/'.$Data['blog']['url'] }}" class="email-button-secondary" target="_blank">Leer Más</a>
            </div>
        </div>
